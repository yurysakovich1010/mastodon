...
export const expandAccountTimeline         = (accountId, { maxId, withReplies, onlyReplies, onlyImage, onlyVideo, statusId, pinned, unpinned } = {}) => expandTimeline(`account:${accountId}${withReplies ? ':with_replies' : ''}${onlyReplies ? ':only_replies' : ''}${onlyImage ? ':only_image' : ''}${onlyVideo ? ':only_video' : ''}${statusId ? ':specific' : ''}${pinned ? ':pinned' : ''}${unpinned ? ':unpinned' : ''}`, `/api/v1/accounts/${accountId}/statuses`, { only_replies: !!onlyReplies, only_video: !!onlyVideo, only_image: !!onlyImage, exclude_replies: !withReplies, max_id: maxId, with_reply_origin: true, status_id: statusId, pinned: pinned, unpinned: unpinned });
